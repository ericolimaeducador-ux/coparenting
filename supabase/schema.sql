...
CREATE POLICY "partnerships_update" ON partnerships FOR UPDATE USING (parent_1_id = auth.uid() OR parent_2_id = auth.uid());
CREATE POLICY "partnerships_delete" ON partnerships FOR DELETE USING (parent_1_id = auth.uid() OR parent_2_id = auth.uid());

-- Allow reading partnerships by invite token (for accepting invites)
CREATE POLICY "partnerships_by_token" ON partnerships FOR SELECT USING (invite_token IS NOT NULL);
